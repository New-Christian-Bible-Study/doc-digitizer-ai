...
class OCREditorApp:
    def __init__(self, root):
        ...
        # Auto-load if passed via command line args
        if len(sys.argv) > 1:
    ...
    def setup_ui(self):
        ...
        tk.Button(row2, text="Auto Scale", command=self.auto_detect_scale).pack(side=tk.LEFT, padx=2)
        ...
        font_scale = tk.Scale(row2, from_=10, to=36, orient=tk.HORIZONTAL, variable=self.font_size_var, showvalue=1, length=100)
        font_scale.pack(side=tk.LEFT)
        font_scale.bind("<ButtonRelease-1>", lambda e: self.refresh_view()) 

        self.serif_font_var = tk.BooleanVar(value=False)
        tk.Checkbutton(row2, text="Serif Font", variable=self.serif_font_var, command=self.refresh_view).pack(side=tk.LEFT, padx=5)

        tk.Button(row2, text="Apply View", command=self.refresh_view).pack(side=tk.LEFT, padx=10)

        tk.Label(row2, text=" | Layout:").pack(side=tk.LEFT, padx=(10, 2))
        ...
        try:
            with open(txt_path, 'w', encoding='utf-8') as f:
                for item in self.json_data:
                    ...
                    lines_to_export = [line.strip() for line in target_texts if line.strip()]
                    ...
                    for i in range(len(lines_to_export)):
                        line = lines_to_export[i]
                        if not line: continue
                        
                        if remove_hyphens and re.search(r'[-‐‑—–]+\s*$', line) and i + 1 < len(lines_to_export):
                            line = re.sub(r'[-‐‑—–]+\s*$', '', line)
                            
                            next_line = lines_to_export[i+1].strip()
                            if next_line:
                                parts = next_line.split(None, 1) 
                                first_word = parts[0]
                                rest_of_next_line = parts[1] if len(parts) > 1 else ""
                                
                                line += first_word 
                                lines_to_export[i+1] = rest_of_next_line 
    ...
    def auto_detect_scale(self):
        if not self.json_data or not self.image_paths: return
        res_node = self.get_res_node(0)
        
        max_x = 0
        for poly in res_node.get('rec_polys', []):
            for p in poly:
                max_x = max(max_x, p[0]) 
        ...
        try:
            with Image.open(self.image_paths[0]) as img:
                scale = max_x / img.width
                self.scale_var.set(round(scale, 4)) 
                self.refresh_view()
        except Exception as e:
    ...
    def create_full_page_widget(self, parent, res_node, img_path):
        ...
        base_font = "Times New Roman" if self.serif_font_var.get() else "Arial"
        custom_font = (base_font, self.font_size_var.get())
